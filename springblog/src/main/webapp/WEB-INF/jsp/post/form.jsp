...
<title>Nrimi's blog / Write</title>
   ...
   <br/>
	...
	<form:form action="${requestScope['javax.servlet.forward.servlet_path']}" commandName="postCommand" id="post" onsubmit="if($('#pen').html()!='<p><br></p>')$('#content').val($('#pen').html()); pen.destroy();" method="post">
		...
		<form:input type="text" path="title" placeholder="Title"
			style="height: 50px; width: 100%; font-size: 35px; 
		...
		<form:input type="text" path="subtitle" placeholder="Subtitle (optional)"
			style="height: 30px; width: 100%; font-size: 14px; 
			border: none; border-right: 0px; border-top: 0px; boder-left: 0px; boder-bottom: 1px; outline-style: none; 
		...
		<form:input type="text" path="tags" placeholder="Tag"
		...
		<a href="/post/list" class="btn btn-default" role="button">취소</a>
	...
	<div id="toolbar"><!-- 
		...
		<form action="/category/add" method="post" id="add_category" >
			<input type="text" name="categoryName" class="form-control" placeholder="새 카테고리" required="required">
